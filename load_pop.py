...
def get_mixing_matrix(sheet_name: str):
    ...
    """
    mixing_matrix = pd.read_excel(dirname + '\\data\\inputs.xlsx', sheet_name=sheet_name, usecols=range(17), index_col=0)
...
def get_australian_popdict(setting='Melbourne', pop_size=100, contact_numbers={'H': 4,'S': 22,'W': 20,'C': 20}):
    ...
    reference = pd.read_excel(dirname + '\\data\\inputs.xlsx', sheet_name = 'household_heads').set_index('Age')[setting]  # Age distribution of reference person
    household_dist = pd.read_excel(dirname + '\\data\\inputs.xlsx', sheet_name='households')['no. households']  # Household size distribution
    ...
    household_dist = (household_dist * (pop_size / sum(household_dist.index * household_dist))).astype(int)
