...
for idx, nt in enumerate([6260]):
    ...
    #ls1.append(ax1.plot(data["cluster_size"], data["resurgence_prob"]/(data["outbreak_prob"]), color=category_colors[2, ...]*0.8, lw=2, label=str(nt)+" [wild strain]"))
    ls2.append(ax1.plot(data["cluster_size"], data["resurgence_prob"], color=category_colors[2, ...]*0.8, lw=2, ls='-',label="P[o] "+str(nt)+" [wild strain]"))
    ls3.append(ax1.plot(data["cluster_size"], data["outbreak_prob"], color=category_colors[2, ...]*0.8, lw=2, ls=':',label= "P[d]" + str(nt)+" [wild strain]"))
...
for idx, nt in enumerate([6260, 8360, 10460, 12560]):
    ...
    ls2.append(ax1.plot(data["cluster_size"], data["resurgence_prob"], color=category_colors[idx+5, ...]*0.8, lw=2, ls='-',label="P[o] "+str(nt)+" [UK variant]"))
...
fig.tight_layout()
#cv.savefig(f"fig_prob_outbreak_oc_de_cluster_tests_iq_0.1_uk-oz.png", dpi=300)
