...
for idx, nt in enumerate([6260]):
    ...
    #ls2.append(ax1.plot(data["cluster_size"], data["resurgence_prob"], color=[0.2,0.2,0.2], lw=2, ls='-',label="P[o] "+str(nt)+" [wild strain]"))
...
ax1.legend(handler1, label1, loc="upper right", frameon=True, title='number of daily tests')
fig.tight_layout()
cv.savefig(f"fig_prob_outbreak_oc_minus_de_cluster_tests_iq_1.0_uk-oz.png", dpi=300)
