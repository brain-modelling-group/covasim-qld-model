...
def plot_heatmaps(df_map_list, fig_name_list, xlab, cmap_name='inferno'):
    ...
    for df_map, fig_name in zip(df_map_list, fig_name_list):
        ...
        sns.heatmap(df_map, annot=True, fmt=".0f", linewidths=.5, ax=ax, cmap=cmap_name, vmin=0, vmax=100)
...
dfcloz_fc_map = get_subframe(df_cloz, num_tests, cluster_col, cluster_size_th, "outbreak_inf_prob") 
dfcluk_fc_map = get_subframe(df_cluk, num_tests, cluster_col, cluster_size_th, "outbreak_inf_prob") 
...
plot_heatmaps([dfcloz_fc_map, dfcluk_fc_map], [f"fig_resurgence_vs_tests_cluster_oz.png", "fig_resurgence_vs_tests_cluster_uk.png"], 'cluster size', cmap_name="viridis")
