...
ax1.set_ylabel('prob resurgence [%]')
ax1.set_xlim([0, 17])
...
for idx, nt in enumerate([4160, 6260, 8360, 10460, 12560]):
    ...
    ls1.append(ax1.scatter(data["cluster_size"], data["resurgence_prob"], color=category_colors[idx, ...], label=f"{nt}"))
...
fig.tight_layout()
cv.savefig(f"fig_prob_resurgence_cluster_tests_iq_1.0.png", dpi=300)
