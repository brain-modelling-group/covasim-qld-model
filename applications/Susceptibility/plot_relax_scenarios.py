...
for statsfile in scendir.rglob('*.stats'):
    ...
    for d in stats:
        ...
        d['package_name'] = '_'.join(statsfile.stem.split('_')[:-1])
        d['n_initial'] = n_initial
    records += stats
df = pd.DataFrame.from_records(records)


package_names = outbreak.load_packages('relax_packages.csv')[1]
scenario_names = outbreak.load_scenarios('relax_scenarios.csv')[1]
...
def bootstrap_quantity(df, quantity, n_bootstrap=1):
    ...
    stds = pd.concat(mean_samples).groupby(['scenario_name', 'package_name', 'n_initial']).std()
    stds.name = 'std'
...
### BOXPLOT OF ACTIVE INFECTIONS

fig, ax = plt.subplots()
sns.boxplot(order=list(package_names.values()),x='package_name',y='active_infections',data=df.loc['Baseline'].reset_index(), fliersize=2,color='#c90000', width=0.5)

medians = df.loc['Baseline'].reset_index().groupby('package_name').median()['active_infections']
for i, package in enumerate(package_names.values()):
    ax.text(i, medians.loc[package], f'{medians.loc[package]:.0f}', ha='center', va='bottom', color='w')
