...
for i,jb in enumerate(maskbetas):
    ...
    msim.reduce()
    w0, w1, w2, w3, w4 = cv.date('2020-08-16'), cv.date('2020-08-23'), cv.date('2020-08-30'), cv.date('2020-09-06'), cv.date('2020-09-13')
...
tlc = sc.loadobj(f'results/nsw_layer_counts.obj')
layer_counts = {}
...
ax2 = pl.axes([x0, y0, dx, dy])
ax2.plot(range(6000), infprobs[0], '-', lw=4, c=colors[0], label="Victorian masks", alpha=1.0)
ax2.plot(range(6000), infprobs[1], '-', lw=4, c=colors[1], label="Moderate masks", alpha=1.0)
ax2.plot(range(6000), infprobs[2], '-', lw=4, c=colors[2], label="No masks", alpha=1.0)
ax2.set_ylim(0,1)
#ax2.minorticks_on()
#pl.grid(which='minor', c=[0, 0, 0], linestyle='--', alpha=0.3, lw=1)
#pl.grid(which='major', c=[0, 0, 0], linestyle='--', alpha=0.3, lw=1)
...
scenlabels = ['No masks', 'Moderate masks', 'Victorian masks']
