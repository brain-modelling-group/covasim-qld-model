...
if __name__ == "__main__":
    ...
    user_pars = {locations[0]: {'pop_size': int(10e4),
                        'beta': 0.09,
                        'n_days': 320,
                        'pop_infected': 0, #at date of first non-zero values for cases, deaths, tests
                        'calibration_end': '2020-08-01'}}

    mid_july = 129 # date with first non-zero value for cases, deaths, tests = day zero, then # to mid-July
    mid_aug = mid_july + 31 
    mid_sep = mid_aug + 31
    mid_oct = mid_sep + 30
    end_oct = mid_sep + 46
    ...
    metapars = {'n_runs': 1, #set to 1 to test, set to 8 to run
                'noise': 0.00, #set to 0 to test, set to 0.03 to run
                'verbose': 1,
                'rand_seed': 1}

    # the policies to change during scenario runs
    # scen_opts = {'BH': {'No changes to current lockdown restrictions':
    scen_opts = {locations[0]: {'Small easing of restrictions in mid-September':
                                   {'replace': (['relax3'], [['relax4']], [[mid_sep]]),
                                    'policies': {'relax3': {'beta': 0.85}}}, #ignore, have not been working, modify in input sheet

                               'Moderate easing of restrictions in mid-September':
                                   {'replace': (['relax3'], [['relax5']], [[mid_sep]]),
                                    'policies': {'relax3': {'beta': 0.95}}},

                               'Small easing of restrictions in mid-October':
                                   {'replace': (['relax3'], [['relax4']], [[mid_sep]]),
                                    'policies': {'relax3': {'beta': 0.85}}},

                               'Moderate easing of restrictions in mid-October':
                                   {'replace': (['relax3'], [['relax5']], [[mid_sep]]),
                                    'policies': {'relax3': {'beta': 0.95}}},
    ...
    utils.policy_plot2(scens, plot_ints=False, do_save=True, do_show=True,
                       fig_path=dirname + '/figs/Mobile-projection' + '.png',
    ...
    # Mid Bound: small change
    new_inf_MB_sep_oct = sum(
        scens['scenarios'][locations[0]].results['new_infections'][
            'Moderate easing of restrictions in mid-September']['best'][
        mid_sep:end_oct])
    ...
    cum_inf_MB_sep_oct = \
        scens['scenarios'][locations[0]].results['cum_infections'][
            'Moderate easing of restrictions in mid-September']['best'][end_oct]
    ...
    cum_inf_MB_nov_dec = \
        scens['scenarios'][locations[0]].results['cum_infections'][
            'Moderate easing of restrictions in mid-September']['best'][end_dec]
    ...
    new_inf_UB_sep_oct = sum(
        scens['scenarios'][locations[0]].results['new_infections'][
            'Moderate easing of restrictions in mid-August']['best'][
        mid_sep:end_oct])
    ...
    cum_inf_UB_sep_oct = \
        scens['scenarios'][locations[0]].results['cum_infections'][
            'Moderate easing of restrictions in mid-August']['best'][end_oct]
    ...
    cum_inf_UB_nov_dec = \
        scens['scenarios'][locations[0]].results['cum_infections'][
            'Moderate easing of restrictions in mid-August']['best'][end_dec]
