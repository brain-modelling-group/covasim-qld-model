...
if __name__ == "__main__":
    ...
    # Mexicoecify layer keys to use
    all_lkeys = ['H', 'S', 'W', 'C']
    dynamic_lkeys = ['C']  # layers which update dynamically (subset of all_lkeys)

    # country-Mexicoecific parameters
    user_pars = {'Mexico': {'pop_size': int(10e4),
                               'beta': 0.0535,
                               'n_days': 366,
    ...
    # scen_opts = {'Mexico': {'No changes to current lockdown restrictions': 
    scen_opts = {'Mexico': {'Small easing of restrictions in end-August': 
    ...
    utils.policy_plot2(scens, plot_ints=False, do_save=True, do_show=True,
                        fig_path=dirname + '/figs/Mexico-projection' + '.png',
    ...
    new_inf_LB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_infections']['No changes to current lockdown restrictions']['best'][mid_sep:end_oct])    
    new_diag_LB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['No changes to current lockdown restrictions']['best'][mid_sep:end_oct])    
    cum_inf_LB_sep_oct = scens['scenarios']['Mexico'].results['cum_infections']['No changes to current lockdown restrictions']['best'][end_oct]    
    ...
    new_inf_LB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_infections']['No changes to current lockdown restrictions']['best'][start_nov:end_dec])    
    new_diag_LB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['No changes to current lockdown restrictions']['best'][start_nov:end_dec])    
    cum_inf_LB_nov_dec = scens['scenarios']['Mexico'].results['cum_infections']['No changes to current lockdown restrictions']['best'][end_dec]    
    ...
    new_inf_MB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_infections']['Small easing of restrictions in end-September']['best'][mid_sep:end_oct])    
    new_diag_MB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['Small easing of restrictions in end-September']['best'][mid_sep:end_oct])    
    cum_inf_MB_sep_oct = scens['scenarios']['Mexico'].results['cum_infections']['Small easing of restrictions in end-September']['best'][end_oct]    
    ...
    new_inf_MB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_infections']['Small easing of restrictions in end-September']['best'][start_nov:end_dec])    
    new_diag_MB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['Small easing of restrictions in end-September']['best'][start_nov:end_dec])    
    cum_inf_MB_nov_dec = scens['scenarios']['Mexico'].results['cum_infections']['Small easing of restrictions in end-September']['best'][end_dec]    
    ...
    new_inf_UB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_infections']['Moderate easing of restrictions in end-August']['best'][mid_sep:end_oct])    
    new_diag_UB_sep_oct = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['Moderate easing of restrictions in end-August']['best'][mid_sep:end_oct])    
    cum_inf_UB_sep_oct = scens['scenarios']['Mexico'].results['cum_infections']['Moderate easing of restrictions in end-August']['best'][end_oct]    
    ...
    new_inf_UB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_infections']['Moderate easing of restrictions in end-August']['best'][start_nov:end_dec])    
    new_diag_UB_nov_dec = sum(scens['scenarios']['Mexico'].results['new_diagnoses']['Moderate easing of restrictions in end-August']['best'][start_nov:end_dec])    
    cum_inf_UB_nov_dec = scens['scenarios']['Mexico'].results['cum_infections']['Moderate easing of restrictions in end-August']['best'][end_dec]    
    ...
    workbook = xlsxwriter.Workbook('Mexico_projections.xlsx')     
